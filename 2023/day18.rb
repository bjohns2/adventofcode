...
def handle_up(map, current_location)
  new_location = [current_location[0] - 1, current_location[1]]
  if new_location[0] < 0
    map.unshift((['.'] * map[0].size))
...
def handle_left(map, current_location)
  new_location = [current_location[0], current_location[1] - 1]
  if new_location[1] < 0
    map.each {|l| l.unshift('.')}
    new_location = current_location
  end
  map[new_location[0]][new_location[1]] = '-'
  new_location
  ...
  map.each_with_index do |line, i|
    ...
    line.each_with_index do |char, j|
      if char == '|'
        ...
        looking_for_up = false
      elsif char == '7' && looking_for_down == true
        currently_inside = !currently_inside
        looking_for_down = false
        looking_for_up = false
      elsif char == 'F'
        looking_for_up = true
        looking_for_down = false
      elsif char == 'J' && looking_for_up == true
        currently_inside = !currently_inside
        looking_for_down = false
        looking_for_up = false
      elsif char != '-'
...
def flood_fill(map, i, j)
  if (map[i][j] != '.')
    return      
  end                                      
 
  # if(map[i][j] != target_color) 
  #     return;
  ...
  flood_fill(map, i, j+1) if j+1 < map[0].size
  flood_fill(map, i, j-1) if j-1 > 0 
  flood_fill(map, i+1, j) if i+1 < map.size
  flood_fill(map, i-1, j) if i-1 > 0 
end
