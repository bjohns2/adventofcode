...
my_input = "jz{m<1130:R,m>1525:A,R}
...
def parse_input(input)
  raw_workflows = input.split("\n\n")[0].split("\n")
  workflows = {}
  raw_workflows.each do |raw_workflow|
    name = raw_workflow.split('{')[0]
    raw_conditions = raw_workflow.split('{')[1][0..-2] # e.g. s>2770:qs,m<1801:hdj,R
    conditions = raw_conditions.split(',').map do |raw_condition| # e.g. s>2770:qs or qqz
      if raw_condition.size < 4
        ['x', '>', 0, raw_condition]
      else
        category = raw_condition[0]
        comparator_operation = raw_condition[1]
        comparator_value = raw_condition.split(':')[0][2..-1].to_i
        destination_workflow = raw_condition.split(':')[1]
        [category, comparator_operation, comparator_value, destination_workflow]
      end
    end
    workflows[name] = conditions
    # puts conditions.map{|c| c.join(',')}.join("\n")
    # conditions
  end
  ...
  parts = raw_parts.map do |raw_part|
    part = {}
    raw_specs = raw_part[1..-2].split(',')
    raw_specs.each do |spec|
      category = spec.split('=')[0]
      category_value = spec.split('=')[1].to_i
      part[category] = category_value
    end
    part
  end
...
def part_accepted?(part, workflows)
  current_workflow = 'in'
  while !['A', 'R'].include?(current_workflow)
    # puts "current_workflow: #{current_workflow}"
    conditions = workflows[current_workflow]
    new_workflow = nil
    conditions.each do |condition| # e.g. ['x', '>', 0, 'asd']
      category = condition[0]
      comparator = condition[1]
      comparator_value = condition[2]
      destination_workflow = condition[3]
      # puts "#{part[category]} #{comparator} #{comparator_value}"
      if new_workflow != nil
        next
      end
      if comparator == '>'
        if part[category] > comparator_value
          new_workflow = destination_workflow
        end
      else # comaprator is <
        if part[category] < comparator_value
          new_workflow = destination_workflow
...
def solution(input)
  ...
  parts = parsed_input[1]
  
  workflows.each {|k,v| puts "#{k}: #{v}"}

  accepted_parts = parts.select do |part|
    accepted = part_accepted?(part, workflows)
    # puts "part: #{part}, accepted: #{accepted}"
    accepted
  end
  total = 0
  accepted_parts.each do |part|
    part.each do |condition, value|
      total += value
    end
  end
  puts total
